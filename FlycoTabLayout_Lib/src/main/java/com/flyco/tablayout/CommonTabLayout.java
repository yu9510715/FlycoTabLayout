...
public class CommonTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    ...
    private void addTab(final int position, View tabView) {
        ...
        if (!TextUtils.isEmpty(mTabEntitys.get(position).getTabUnSelectedStr())) {
            Glide.with(mContext).load(mTabEntitys.get(position).getTabUnSelectedStr()).into(iv_tab_icon);
        } else {
...
//            tv_tab_title.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
            ...
            if (mIconVisible) {
                ...
                if(!TextUtils.isEmpty(tabEntity.getTabUnSelectedStr())){
                    Glide.with(mContext).load(i == mCurrentTab?tabEntity.getTabSelectedStr():tabEntity.getTabUnSelectedStr()).into(iv_tab_icon);
                }else {
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            CustomTabEntity tabEntity = mTabEntitys.get(i);
            if(!TextUtils.isEmpty(tabEntity.getTabUnSelectedStr())){
                Glide.with(mContext).load(isSelect?tabEntity.getTabSelectedStr():tabEntity.getTabUnSelectedStr()).into(iv_tab_icon);
            }else {
